...
function loadCurrentPage(force=false){if(state.activePage==='ads')return loadAds(force);if(state.activePage==='stocks')return loadStocks(force);if(state.activePage==='fbw-stocks')return loadFbwStocks(force);if(state.activePage==='prices')return loadPrices(force);if(state.activePage==='fbs-orders')return loadFbsOrders(force);if(state.activePage==='funnel')return loadFunnelDetails();return loadDashboard()}
...
async function loadPriceStocks(force=false){
  ...
  try{
    ...
    $('#alertArea').insertAdjacentHTML('beforeend',`<div class="alert">Остатки: ${escapeHtml(e.message)}</div>`);
  }
  mergePriceStocks();
...
function funnelProductRows(){
  ...
  return (state.funnelProducts||[]).map(item=>{
    const p=item.product||item,s=item.statistic?.selected||item.history?.[0]||item,c=s.conversions||{},stocks=p.stocks||{};
    return {...p,...s,title:p.title||p.name||`Товар ${p.nmId}`,vendorCode:p.vendorCode||'',subjectName:p.subjectName||'Без категории',nmId:p.nmId,
      addToCartPercent:Number(c.addToCartPercent??s.addToCartConversion??0),cartToOrderPercent:Number(c.cartToOrderPercent??s.cartToOrderConversion??0),
      buyoutPercent:Number(c.buyoutPercent??s.buyoutPercent??0),addToWishlist:Number(s.addToWishlist??s.addToWishlistCount??0),
      orderSum:Number(s.orderSum||0),buyoutSum:Number(s.buyoutSum||0),stock:Number(stocks.wb||0)+Number(stocks.mp||0)};
...
function renderFunnelDetails(){
  ...
  $('#funnelProductsBody').innerHTML=pageRows.map(row=>`<tr><td class="funnel-product-cell">${row.photo?`<img class="stock-product-photo" src="${escapeHtml(row.photo)}" alt="" loading="lazy">`:''}<strong>${escapeHtml(row.title)}</strong></td>`+
    `<td>${escapeHtml(row.vendorCode||'—')}</td><td>${escapeHtml(row.nmId||'—')}</td><td>${escapeHtml(row.subjectName)}</td>`+
    `<td>${fmtNum(row.openCount)}</td><td>${fmtNum(row.cartCount)}</td><td>${fmtPercent(row.addToCartPercent)}</td><td>${fmtNum(row.addToWishlist)}</td>`+
    `<td>${fmtNum(row.orderCount)}</td><td>${fmtPercent(row.cartToOrderPercent)}</td><td>${fmtRub(row.orderSum)}</td>`+
    `<td>${fmtNum(row.buyoutCount)}</td><td>${fmtRub(row.buyoutSum)}</td><td>${fmtPercent(row.buyoutPercent)}</td></tr>`).join('')||
...
function renderDashboardAlerts(){
  ...
  area.querySelectorAll('.dashboard-alert').forEach(el=>el.remove());
